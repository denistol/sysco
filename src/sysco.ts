...
import { SyscoSearchResponse } from './types';
import { getCookies } from './getCookie';
import { uniqBy } from 'lodash';
import { delay, write } from './utils';
...
const fetchPageWithRetry = async (
    ...
    while (retries <= maxRetries) {
        try {
            ...
            const data = await res.json() as SyscoSearchResponse;
            return data;
...
export const load = async () => {
    ...
    const currentCookie = await getCookies.sysco()
    const pages = new Array(PARSE_PAGES).fill(null).map((el, index) => index * PER_PAGE)
    let products: Product[] = []
    let categoryList: Category[] = []

    await write('data/sysco/brands', currentCookie.brands.map((brand, index) => ({index, brand})))

    try {
        for (const startPage of pages) {
            const out = await fetchPageWithRetry(startPage, getCookies.sysco, currentCookie.cookieString, 3);
        ...
        await write('data/sysco/products', products)
        await write('data/sysco/categories', uniqBy(categoryList, 'id') as object[])
    }
